...
def get_feats(
    ...
    X_unscaled = X_unscaled[core.config.features_list]

    return pd.DataFrame(
        data=scaler.transform(X_unscaled),
        columns=core.config.features_list,
        index=[0],
    )
...
def get_advise(input: pd.DataFrame, model) -> pd.DataFrame:
    ...
    try:
        df_result = pd.DataFrame(
            {
                "predict_P1_wins": model.predict(X),
                "certainty": np.absolute(2 * model.predict_proba(X)[:, 1] - 1.0),
            },
            index=X.index,
        )
    except TypeError:
        df_result = pd.DataFrame(
            {
                # "predict_P1_wins": model.predict(X),
                "predict_P1_wins": model.predict(X)[:, 0] > .5,
                "certainty": np.absolute(2 * model.predict(X)[:, 0] - 1.0),
            },
            index=X.index,
        )
